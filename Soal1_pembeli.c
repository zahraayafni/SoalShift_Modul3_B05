...
void main()
{
	...
	if(pil==1){
		...
		if(strcmp(barang,"MP4A1")==0 && *mp4a1 > 0) *mp4a1 -= amount;
		else if(strcmp(barang,"PM2_V1")==0 && *pm2_v1 > 0) *pm2_v1 -= amount;
		else if(strcmp(barang,"SPR_3")==0 && *spr_3 > 0) *spr_3 -= amount;
		else if(strcmp(barang,"SS2_V5")==0 && *ss2_v5 > 0) *ss2_v5 -= amount;
		else if(strcmp(barang,"SPG1_V3")==0 && *spg1_v3 > 0) *spg1_v3 -= amount;
		else if(strcmp(barang,"MINE")==0 && *mine > 0) *mine -= amount;
		else printf("Maaf, stok barang tidak tersedia\n");
		}
